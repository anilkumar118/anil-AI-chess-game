...
class Engine:
    ...
    def minimax(self, level, move, maximiser):
        if level == 0:
        ...
        if maximiser:
            ...
            for move in moves:
                ...
                new_move, new_score = self.minimax(level - 1, move, False)
        ...
        if not maximiser:
            ...
            for move in moves:
                self.leaves_reached += 1
                self.board.push(move)
                new_move, new_score = self.minimax(level - 1, move, True)
                if new_score < best_score:
                    best_score, best_move = new_score, move
                self.board.pop()
    ...
    def alpha_beta(self, level_neg, level_pos, move, alpha, beta, prev_moves, maximiser):
        ...
        if prev_moves and len(prev_moves) >= level_neg:
            if level_neg == 4 and not self.board.turn:
                print(prev_moves[level_neg - 1])
            if prev_moves[level_neg - 1] in moves:
           
                moves.insert(0, prev_moves[level_neg - 1])
        ...
        if maximiser:
            for move in moves:
                ...
                self.board.push(move)
                new_sequence, new_score = self.alpha_beta(level_neg - 1, level_pos + 1, move, alpha, beta, prev_moves, False)
                self.board.pop()
        ...
        if not maximiser:
            for move in moves:
                ...
                self.board.push(move)
                new_sequence, new_score = self.alpha_beta(level_neg - 1, level_pos + 1, move, alpha, beta, prev_moves, True)
                self.board.pop()
    ...
    def calculate_minimax(self, level):
        
        maximiser = self.board.turn

        best_move, best_score = self.minimax(level, None, maximiser)

        return str(best_move)


    def calculate_ab(self, level):
        maximiser = self.board.turn

        move_sequence, best_score = self.alpha_beta(level, 0, None, -10000001, 10000001, None, maximiser)
    ...
    def iterative_deepening(self, level):
        ...
        print("level calculated:", len(move_list))
